...
class PocHMM(MetaHMM):
    ...
    def infer_missing_af(self, bafs, geno, eps=0.1):
        """Estimate population allele frequency for the unobserved parent from BAF.

        .. note::
            Not yet implemented.  Planned to use the embryo BAF at homozygous
            observed-parent sites to impute a local allele frequency prior for
            the missing parent.

        Arguments:
            - bafs (`np.array`): m-length array of B-allele frequencies
            - geno (`np.array`): m-length array of called genotypes for the observed parent
            - eps (`float`): genotyping error rate used when evaluating the BAF likelihood

        Raises:
            NotImplementedError: always; method is not yet implemented.
        """
